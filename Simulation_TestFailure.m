...
for jj=1:2
    ...
    if jj == 1 
        opt.nonuniform_grid=0;
    elseif jj==2 % if use non-uniform grid discretization (go to InitMemory.m, line 436 to configure)
    ...
    [mem, input] = InitMemory(settings, opt, input);

    %% Simulation (start your simulation...)

    mem.iter = 1; time = 0.0;
    Tf = Ts;  % simulation time
    state_sim= input.x0';
        ...
        nextTime = mem.iter*Ts; 
         if strcmp(settings.model,'InvertedPendulum')
            if mem.iter==1
                figure(2)
                hold on
                title('Position (Prediction Horizon, First step)')
                grid on
                if opt.nonuniform_grid==0 
                    plot(0:N,output.x(1,:),'r','Linewidth',1)
                    Ts1 = ones(1,N)*Ts;
                    v  = diff(output.x(1,:))./Ts1(1:end);
                    a = diff(v)./Ts1(2:end);
                else
                    plot(mem.index_T(:),output.x(1,:),'b','Linewidth',1)
                    Ts1 = diff([0,1:1:40,42:2:80])*Ts;
                    v  = diff(output.x(1,:))./Ts1(1:end);
                    a = diff(v)./Ts1(2:end);
                end
                ...
                hold on
                grid on
                title('Force (Prediction Horizon, First step)')
                if opt.nonuniform_grid==0
                    stairs(1:N,output.u(1,:),'r','Linewidth',1)
                    stairs(2:N,a,'g--','Linewidth',1)
                else
                    stairs(mem.index_T(2:end),output.u(1,:),'b','Linewidth',1)
                    stairs(mem.index_T(3:end),a,'k--','Linewidth',1)
                end
            end
        end
        mem.iter = mem.iter+1;
...
legend('P_{UG}','P_{NUG}')
figure(3)
legend('F_{UG}','acc_{UG_{diffdiff}}','F_{NUG}','acc_{NUG_{diffdiff}}')
%%
...
if opt.nonuniform_grid==1
    ...
    figure(2)
    plot(mem.index_T(:),xf(1,:),'g--','Linewidth',1)
    hold on
    plot(mem.index_T(:),xf2(1,:),'c--','Linewidth',1)
    %title('Position (Prediction Horizon)')
    legend('P_{UG}','P_{NUG}','Double integration ts>40','Single integration ts>40')
    grid on
end
